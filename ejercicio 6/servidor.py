...
def configuracion():
    ...
    global conexion
    global cursor
    conexion = mysql.connector.connect(user="root", password="", host="localhost", database="triki")
...
def encargarse_cliente(cliente_loc):
    ...
    bienvenido = "Bienvenido %s! \nEspera hasta que haya otro jugador en linea, o escribe {salir} para salir." % nombre
    cliente_loc.send(bytes(bienvenido, "utf-8"))

    mensaje = "%s se ha unido al juego." % nombre
    broadcast(bytes(mensaje, "utf-8"))
    clientes[cliente_loc] = nombre

    if len(clientes) == 2:
        mens = "[PLAY]"
        broadcast(bytes(mens, "utf-8"))
    ...
    while True:
        ...
        if mensaje != bytes("{salir}", "utf-8"):
            ...
            broadcast(bytes("%s ha salido del juego." % nombre, "utf-8"))
            break
    ...
    try:
        ...
        if len(rows) == 0:
            ...
            try:
                ...
                enviar_historico(cliente, cliente_loc)
            except:
                print("hubo un error")
...
def enviar_historico(cliente, cliente_loc):
    sql = "SELECT COUNT(*) FROM juegos WHERE win_user = '%s'" % cliente
    cursor.execute(sql)
    rows = cursor.fetchall()
    victorias=0
    if len(rows) > 0:
    ...
    sql = "SELECT COUNT(*) FROM juegos WHERE loser_user = '%s'" % cliente
    cursor.execute(sql)
    rows = cursor.fetchall()
    derrotas=0
    if len(rows) > 0:
    ...
    mensajes_juegos = "Victorias = "+str(victorias)+", Derrotas = "+str(derrotas)
    cliente_loc.send(bytes(mensajes_juegos, "utf-8"))
